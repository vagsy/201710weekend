...
function render(element,container){
   ...
   for(let attr in element.props){
     if(attr == 'children'){//如果是子元素的特殊处理
         ...
         //如果子元素们是一个数组，则依次把这些子元素变成真实DOM元素并添加到自己身上
          ...
          element.props.children.forEach(item=>{
            if(typeof item =='string'){
              let textNode = document.createTextNode(item);//则创建文件节点关追加到节点上
              parent.appendChild(textNode);
            }else{
              render(item,parent);
            }
       ...
       for(let key in element.props.style){//color height
         parent.style[key] = element.props.style[key];
       }
     }else if(attr == 'className'){
       parent.className = element.props.className;
     }else{
       //设置其它属性
